class DonationNotificationJob < ApplicationJob
  queue_as :default

  def perform(donation_id, notification_type)
    ...
    Rails.logger.error "DonationNotificationJob: Donation not found - #{e.message}"
  rescue StandardError => e
    Rails.logger.error "DonationNotificationJob: #{e.class} - #{e.message}"
    raise
  end
  ...
  def send_evidence_uploaded_notification
    evidence = donation.latest_payment_evidence
  ...
  def format_paid_message
    ...
    <<~MESSAGE
      ...
      *Date*
      ...
      #{donation.state_updated_at.strftime("%Y-%m-%d %H:%M")}
  ...
  def format_failed_message
    reason = donation.state == "expired" ? "Payment Expired" : "Payment Failed"
  ...
  def manual_payment_with_evidence?
    Current.settings["payment_provider"] == "manual" && donation.latest_payment_evidence&.file&.attached?
  end
