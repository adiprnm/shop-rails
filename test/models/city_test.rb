...
class CityTest < ActiveSupport::TestCase
  ...
  test "validates rajaongkir_id presence" do
    city = City.new(name: "Jakarta Barat", province: provinces(:jawa_barat))
    assert_not city.valid?
    assert_includes city.errors[:rajaongkir_id], "can't be blank"
  end

  test "validates rajaongkir_id uniqueness" do
    existing = cities(:jakarta_barat)
    city = City.new(rajaongkir_id: existing.rajaongkir_id, name: "Jakarta Timur", province: provinces(:jawa_barat))
    assert_not city.valid?
    assert_includes city.errors[:rajaongkir_id], "has already been taken"
  end

  test "validates name presence" do
    city = City.new(rajaongkir_id: "1", province: provinces(:jawa_barat))
    assert_not city.valid?
